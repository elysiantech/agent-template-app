...
import {
  Send, Square as Stop, ChevronRight, ChevronLeft,
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (attachments.length) {
      try {
        ...
        attachments.forEach(({ file }) => formData.append("files", file)); // Append all files

        const response = await fetch("/api/files/upload", { method: "POST", body: formData });
        if (!response.ok) throw new Error("Failed to upload files");

        const { files } = await response.json();
        uploadedAttachments.push(...files);
      } catch (error: any) {
    ...
    handleSubmit(e, { experimental_attachments: uploadedAttachments.length > 0 ? uploadedAttachments : undefined })
    clearAttachments()
  }
